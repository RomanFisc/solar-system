...
//2
let neptuneMoon = solarSystem[7].moons
//console.log(neptuneMoon)

for (let i = 0; i < neptuneMoon.length; i++) {
  if (neptuneMoon[i] === "Nereid") {
    console.log(neptuneMoon[i]);
  }
}

//3
let venusMoon = solarSystem[1].moons
venusMoon.push("Endor")
...
Object.assign(earth, {diameter: "7,917.5"} )
//console.log(earth)
...
//7
let uranus = solarSystem[6].moons

for (i = 0; i < uranus.length; i++) {
  if (uranus[i] === "Umbriel") {
    uranus.pop()
    uranus.push("Oberon")
  }
...
const satPlanets = ['jupiter', 'saturn', 'uranus', 'neptune']

let sum = 0
satPlanets.forEach(planet => {
  const satellites = parseFloat(system[planet].satellites)
  sum += satellites
}) 
